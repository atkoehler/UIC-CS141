...
Deck Deck::operator+(const Deck &addMe) const
{
    ...
    // iterate over Deck (implicit)     
    // add everything on to newDeck
    for (const auto &c : dk)
    {
        newDeck.addCardBottom(c);
    }

    // iterate over Deck two
    // add everything from two on to newDeck
    for (size_t i=0; i < addMe.deckSize(); ++i)
    {
        newDeck.addCardBottom(addMe.dk.at(i));
    }
