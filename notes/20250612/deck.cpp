...
{   
    ...
    // use the vector class assignment operator
    // to copy over all the contents of the vector
    // that is a private data member dk
    newDeck.dk = dk;

    // // iterate over implicit Deck and place cards
    // // into the newDeck
    // for(const auto& e: dk)
    // {
    //     newDeck.addCardBottom(e);
    // }

    // iterate over rhs Deck and place cards
    // into the newDeck
    for(const auto& e: rhs.dk)
    {
        newDeck.addCardBottom(e);
    }
